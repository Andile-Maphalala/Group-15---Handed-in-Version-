...
export class MaintenanceService {
  ...
  GetAssignedJobs(): Observable<[]> {  
    return this.http.get<[]>(this.url + 'api/Maintenance/GetAssignedJobs');  
  }  
  ...
  updateJob(employee: Job): Observable<Job> {  debugger;
    const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json'}) };  
    return this.http.put<Job>(this.url + 'api/Maintenance/UpdateJobDetails/',  
    employee, httpOptions);  
  }  
...
CreateDateTimeSlots(dateslot: DateTimeSlot): Observable<DateTimeSlot> {   
  const httpOptions = { headers: new HttpHeaders({ 'Content-Type': 'application/json'}) };  
